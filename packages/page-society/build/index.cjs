...
function sortMembers(a, b) {
  ...
  return a.isHead !== b.isHead ? a.isHead ? -1 : 1 : a.isFounder !== b.isFounder ? a.isFounder ? -1 : 1 : a.isSkeptic !== b.isSkeptic ? a.isSkeptic ? -1 : 1 : isVoterA !== (b.isCandidateVoter || b.isDefenderVoter) ? isVoterA ? -1 : 1 // : a.isDefenderVoter !== b.isDefenderVoter
  //   ? (a.isDefenderVoter ? -1 : 1)
  //   : a.isCandidateVoter !== b.isCandidateVoter
  //     ? (a.isCandidateVoter ? -1 : 1)
  : a.isSuspended !== b.isSuspended ? a.isSuspended ? -1 : 1 : a.isWarned !== b.isWarned ? a.isWarned ? -1 : 1 : b.strikes.cmp(a.strikes) || b.payouts.length - a.payouts.length;
}

function getMapMembers(members, skeptics, voters, _ref, warnStrikes) {
  let {
    defender,
    founder,
    hasDefender,
  ...
  const mapMembers = members.filter(member => !hasDefender || !member.accountId.eq(defender)).map(_ref2 => {
    let {
      accountId,
      isDefenderVoter,
      isSuspended,
  ...
  return [mapMembers, mapMembers.reduce((total, _ref3) => {
    let {
      payouts
    } = _ref3;
    return payouts.reduce((total, _ref4) => {
      let [, balance] = _ref4;
      return total.iadd(balance);
    }, total);
  }, new _bn.default(0))];
}

function SocietyApp(_ref5) {
  let {
    basePath,
    className
  } = _ref5;
