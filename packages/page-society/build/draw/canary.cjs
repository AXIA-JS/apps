...
exports.SIZE = exports.PADD = void 0;
...
function addressToBits(publicKey) {
  return publicKey.reduce((bits, byte) => {
    for (let j = 0; j < 8; ++j) {
...
function splitRows(bits, rows) {
  let i = 0; // eslint-disable-next-line no-return-assign

  return rows.map(r => bits.slice(i, i += r));
}

function tattoo(ctx, bits) {
