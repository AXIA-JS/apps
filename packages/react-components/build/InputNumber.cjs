...
class TokenUnit {
  static setAbbr() {
    let abbr = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : TokenUnit.abbr;
    TokenUnit.abbr = abbr;
  }
...
function getSiOptions(symbol, decimals) {
  return _util.formatBalance.getOptions(decimals).map(_ref => {
    let {
      power,
      text,
      value
...
function getValues(api) {
  let value = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : _util.BN_ZERO;
  let si = arguments.length > 2 ? arguments[2] : undefined;
  let bitLength = arguments.length > 3 ? arguments[3] : undefined;
  let isZeroable = arguments.length > 4 ? arguments[4] : undefined;
  let maxValue = arguments.length > 5 ? arguments[5] : undefined;
  let decimals = arguments.length > 6 ? arguments[6] : undefined;
