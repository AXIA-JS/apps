...
function redirect(basePath, location, items, hidden) {
  if (location.pathname !== basePath) {
    ...
    } else if (hidden && (hidden.includes(section) || !items.some(_ref2 => {
      let {
        isRoot,
        name
      } = _ref2;
      return !isRoot && name === section;
    }))) {
...
function Tabs(_ref3) {
  ...
  const filtered = hidden ? items.filter(_ref4 => {
    let {
      name
    } = _ref4;
