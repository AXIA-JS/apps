...
function groupLocks(t, bestNumber, locks = []) {
  return {
    maxBalance: (0, _util.bnMax)(...locks.map(({
      balance
    }) => balance).filter(b => !!b)),
    sorted: locks.map(info => [info, info.unlockAt && info.unlockAt.gt(bestNumber) ? info.unlockAt.sub(bestNumber) : _util.BN_ZERO]).sort((a, b) => (a[0].referendumId || _util.BN_ZERO).cmp(b[0].referendumId || _util.BN_ZERO)).sort((a, b) => a[1].cmp(b[1])).sort((a, b) => a[0].isFinished === b[0].isFinished ? 0 : a[0].isFinished ? -1 : 1).reduce((sorted, [{
      balance,
      isDelegated,
      isFinished = false,
      referendumId,
