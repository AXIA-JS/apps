...
function extractTotals(stakingInfo, progress) {
  ...
  const mapped = stakingInfo.unlocking.filter(_ref => {
    let {
      remainingEras,
      value
    } = _ref;
    return value.gt(_util.BN_ZERO) && remainingEras.gt(_util.BN_ZERO);
  }).map(unlock => [unlock, unlock.remainingEras, unlock.remainingEras.sub(_util.BN_ONE).imul(progress.eraLength).iadd(progress.eraLength).isub(progress.eraProgress)]);
  const total = mapped.reduce((total, _ref2) => {
    let [{
      value
    }] = _ref2;
...
function StakingUnbonding(_ref3) {
  let {
    className = '',
    iconPosition = 'left',
    stakingInfo
