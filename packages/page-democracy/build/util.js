...
import { BN_ONE, BN_TEN, BN_ZERO } from '@axia-js/util';
...
function getDiffs(votes, total, change, inc, totalInc, direction) {
  ...
  const totalChange = BN_ONE.muln(totalInc * 10).mul(voteChange).div(BN_TEN); // return the change, vote with change applied and the total with the same. For the total we don't want
...
function calcChangeAye(threshold, sqrtElectorate, {
  votedAye,
  votedNay,
  votedTotal
}, isPassing, changeAye, inc) {
  while (true) {
...
function calcChangeNay(threshold, sqrtElectorate, {
  votedAye,
  votedNay,
  votedTotal
}, isPassing, changeNay, inc) {
  while (true) {
    // if this one is passing, we only adjust the convictions (since it goes down), if it is failing
    // we assume new votes needs to be added, do those at 1x conviction
...
export function approxChanges(threshold, sqrtElectorate, state) {
  const isPassing = calcPassing(threshold, sqrtElectorate, state); // simple case, we have an aye > nay to determine passing
