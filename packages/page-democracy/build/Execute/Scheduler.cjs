...
const transformEntries = {
  transform: entries => {
    return entries.filter(_ref => {
      let [, vecSchedOpt] = _ref;
      return vecSchedOpt.some(schedOpt => schedOpt.isSome);
    }).reduce((items, _ref2) => {
      let [key, vecSchedOpt] = _ref2;
      const blockNumber = key.args[0];
      return vecSchedOpt.filter(schedOpt => schedOpt.isSome).map(schedOpt => schedOpt.unwrap()).reduce((items, _ref3, index) => {
        let {
          call,
          maybeId,
          maybePeriodic,
...
function Schedule(_ref4) {
  ...
  const filtered = (0, _react.useMemo)(() => bestNumber && items && items.filter(_ref5 => {
    let {
      blockNumber
    } = _ref5;
    return blockNumber.gte(bestNumber);
  }), [bestNumber, items]);
