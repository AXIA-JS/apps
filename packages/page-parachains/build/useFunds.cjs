...
function createResult(bestNumber, minContribution, funds, leased, prev) {
  const [activeRaised, activeCap, totalRaised, totalCap] = funds.reduce(([ar, ac, tr, tc], {
    ...
    isWinner
  }) => [bestNumber.gt(end) || isWinner ? ar : ar.iadd(raised), bestNumber.gt(end) || isWinner ? ac : ac.iadd(cap), tr.iadd(raised), tc.iadd(cap)], [new _bn.default(0), new _bn.default(0), new _bn.default(0), new _bn.default(0)]);
