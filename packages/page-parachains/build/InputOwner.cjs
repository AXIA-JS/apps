...
function InputOwner({
  ...
  const owners = (0, _react.useMemo)(() => ownedIds.map(({
    manager
  }) => manager), [ownedIds]);
  const optIds = (0, _react.useMemo)(() => ownedIds.filter(({
    manager
  ...
  const _setParaId = (0, _react.useCallback)(id => setParaId(noCodeCheck || ownedIds.some(({
    hasCode,
    paraId
