...
function extractEventDetails(events) {
  return events ? events.reduce((_ref, _ref2) => {
    let [deposits, transfers, weight] = _ref;
    let {
      record: {
        event: {
          data,
    ...
    return [section === 'balances' && method === 'Deposit' ? deposits.iadd(data[1]) : deposits, section === 'balances' && method === 'Transfer' ? transfers.iadd(data[2]) : transfers, section === 'system' && ['ExtrinsicFailed', 'ExtrinsicSuccess'].includes(method) ? weight.iadd((method === 'ExtrinsicSuccess' ? data[0] : data[1]).weight) : weight];
  }, [new _bn.default(0), new _bn.default(0), new _bn.default(0)]) : [];
}
