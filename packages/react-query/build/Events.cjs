...
exports.EventsContext = exports.Events = void 0;
...
async function manageEvents(api, prev, records, setState) {
  const newEvents = records.map((record, index) => ({
    ...
    return section !== 'system' && (!['balances', 'treasury'].includes(section) || !['Deposit'].includes(method)) && (!['parasInclusion', 'inclusion'].includes(section) || !['CandidateBacked', 'CandidateIncluded'].includes(method));
  }).reduce((combined, e) => {
    const prev = combined.find(_ref2 => {
      let {
        record: {
          event: {
            method,
      ...
      return e.record.event.section === section && e.record.event.method === method;
    });
