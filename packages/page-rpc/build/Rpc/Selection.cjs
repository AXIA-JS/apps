...
function Selection(_ref) {
  ...
  const params = (0, _react.useMemo)(() => rpc.params.map(_ref2 => {
    let {
      isOptional,
      name,
      type
  ...
  const _nextState = (0, _react.useCallback)(newState => setState(prevState => {
    ...
    const reqCount = rpc.params.reduce((count, _ref3) => {
      let {
        isOptional
      } = _ref3;
      return count + (isOptional ? 0 : 1);
    }, 0);
  ...
  const _onSubmit = (0, _react.useCallback)(() => queueRpc({
    rpc,
    values: values.filter(_ref4 => {
      let {
        value
      } = _ref4;
      return !(0, _util.isNull)(value);
    }).map(_ref5 => {
      let {
        value
      } = _ref5;
