...
function useBlockCounts(accountId, sessionRewards) {
  ...
  (0, _react.useEffect)(() => {
    ...
    if ((0, _util.isFunction)((_api$query$imOnline2 = api.query.imOnline) === null || _api$query$imOnline2 === void 0 ? void 0 : _api$query$imOnline2.authoredBlocks) && sessionRewards && sessionRewards.length) {
      const filtered = sessionRewards.filter(_ref => {
        let {
          sessionIndex
        } = _ref;
        return sessionIndex.gt(_util.BN_ZERO);
      });

      if (filtered.length) {
        Promise.all(filtered.map(_ref2 => {
          let {
            parentHash,
            sessionIndex
          } = _ref2;
          return api.query.imOnline.authoredBlocks.at(parentHash, sessionIndex.sub(_util.BN_ONE), accountId);
        })).then(historic => {
