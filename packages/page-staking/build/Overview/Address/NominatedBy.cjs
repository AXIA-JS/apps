...
function extractChilled() {
  let nominators = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
  let slashingSpans = arguments.length > 1 ? arguments[1] : undefined;
  const chilled = slashingSpans ? nominators.filter(_ref => {
    let {
      submittedIn
    } = _ref;
    return !slashingSpans.lastNonzeroSlash.isZero() && slashingSpans.lastNonzeroSlash.gte(submittedIn);
  }).map(_ref2 => {
    let {
      nominatorId
    } = _ref2;
  ...
  return {
    active: extractFunction(nominators.filter(_ref3 => {
      let {
        nominatorId
      } = _ref3;
      return !chilled.includes(nominatorId);
    }).map(_ref4 => {
      let {
        nominatorId
      } = _ref4;
