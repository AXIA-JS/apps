...
function extractStashes(allRewards) {
  return Object.entries(allRewards).map(([stashId, rewards]) => ({
    available: rewards.reduce((result, {
      ...
      value
    }) => result.iadd(value), result), new _bn.default(0)),
    rewards,
    stashId
  })).filter(({
    available
...
function getAvailable(allRewards) {
  if (allRewards) {
    ...
    const stashAvail = stashes.length ? stashes.reduce((a, {
      available
    }) => a.iadd(available), new _bn.default(0)) : null;
    const [valAvail, valTotal] = validators.length ? validators.reduce(([a, t], {
      available,
      total
    }) => [a.iadd(available), t.iadd(total)], [new _bn.default(0), new _bn.default(0)]) : [null, null];
    return {
      stashAvail,
      stashes,
      valAvail,
  ...
  return /*#__PURE__*/(0, _jsxRuntime.jsxs)("div", {
    ...
    children: [/*#__PURE__*/(0, _jsxRuntime.jsxs)(_reactComponents.Button.Group, {
      ...
      children: !isLoadingRewards && validators.filter(({
        available
      }) => !available.isZero()).map(payout => /*#__PURE__*/(0, _jsxRuntime.jsx)(_Validator.default, {
        isDisabled: isInElection,
        payout: payout
      }, payout.validatorId))
    })]
