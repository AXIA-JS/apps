...
function extractSingle(api, allAccounts, derive, favorites, _ref, historyDepth, withReturns) {
  let {
    activeEra,
    eraLength,
    lastEra,
  ...
  const list = derive.info.map(_ref2 => {
    let {
      accountId,
    ...
    const minNominated = (exposure.others || []).reduce((min, _ref3) => {
      let {
...
function extractBaseInfo(api, allAccounts, electedDerive, waitingDerive, favorites, totalIssuance, lastEraInfo, historyDepth) {
  ...
  const activeTotals = elected.filter(_ref4 => {
    let {
      isActive
    } = _ref4;
    return isActive;
  ...
  const commValues = validators.map(_ref6 => {
    let {
      commissionPer
    } = _ref6;
  ...
  const waitingIds = waiting.map(_ref7 => {
    let {
      key
    } = _ref7;
    return key;
  });
  const validatorIds = (0, _util.arrayFlatten)([elected.map(_ref8 => {
    let {
      key
    } = _ref8;
    return key;
  }), waitingIds]);
  const nominateIds = (0, _util.arrayFlatten)([elected.filter(_ref9 => {
    let {
      isBlocking
    } = _ref9;
    return !isBlocking;
...
const transformMulti = {
  defaultValue: {},
  transform: _ref14 => {
    let [historyDepth, counterForNominators, counterForValidators, optMaxNominatorsCount, optMaxValidatorsCount, minNominatorBond, minValidatorBond, totalIssuance] = _ref14;
    return {
