...
function Nominate(_ref) {
  ...
  const stashes = (0, _react.useMemo)(() => (ownNominators || []).map(_ref2 => {
    let {
      stashId
    } = _ref2;
    return stashId;
  ...
  const _onChangeStash = (0, _react.useCallback)(accountId => {
    const acc = ownNominators && ownNominators.find(_ref3 => {
      let {
        stashId
      } = _ref3;
      return stashId === accountId;
    });
    setIds(acc ? {
