...
function Validator(_ref) {
  ...
  const slashes = (0, _react.useMemo)(() => (allSlashes || []).map(_ref2 => {
    let [era, all] = _ref2;
    return {
      era,
      slashes: all.filter(_ref3 => {
        let {
          validator
        } = _ref3;
        return validator.eq(info.accountId);
      })
    };
  }).filter(_ref4 => {
    let {
    ...
    children: [/*#__PURE__*/(0, _jsxRuntime.jsxs)("td", {
      ...
      children: [/*#__PURE__*/(0, _jsxRuntime.jsx)(_Favorite.default, {
        ...
        hover: t('Slashed in era {{eras}}', {
          replace: {
            eras: slashes.map(_ref5 => {
              let {
                era
              } = _ref5;
              return (0, _util2.formatNumber)(era);
            }).join(', ')
          }
        }),
