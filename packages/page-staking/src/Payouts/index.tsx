...
import type { DeriveStakerReward } from '@axia-js/api-derive/types';
    ...
    <div className={className}>
      ...
      {/* {!isLoadingRewards && !stashes?.length && (
        <article className='warning centered'>
          <p>{t('Payouts of rewards for a validator can be initiated by any account. This means that as soon as a validator or nominator requests a payout for an cycle, all the nominators for that validator will be rewarded. Each user does not need to claim individually and the suggestion is that validators should claim rewards for everybody as soon as an cycle ends.')}</p>
          <p>{t('If you have not claimed rewards straight after the end of the cycle, the validator is in the active set and you are seeing no rewards, this would mean that the reward payout transaction was made by another account on your behalf. Always check your favorite explorer to see any historic payouts made to your accounts.')}</p>
        </article>
      )} */}
      <Table
