...
import AllychainInfo from './AllychainInfo';
...
function Allychain ({ bestNumber, channelDst, channelSrc, className = '', id, lastBacked, lastInclusion, lastTimeout, leasePeriod, nextAction, sessionValidators, validators }: Props): React.ReactElement<Props> {
  ...
  return (
    <tr className={className}>
      ...
      <td className='start'>
        {paraInfo.updateAt && bestNumber && paraInfo.lifecycle?.isAllychain
          ? (
            <>
              {t<string>('Upgrading')}
...
export default React.memo(Allychain);
