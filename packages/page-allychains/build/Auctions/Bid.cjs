...
function Bid(_ref) {
  ...
  const rangeOpts = (0, _react.useMemo)(() => {
    ...
    return ranges.map((_ref2, value) => {
      let [first, last] = _ref2;
      return {
  ...
  const currentWinner = (0, _react.useMemo)(() => lastWinners && lastWinners.winners.find(_ref3 => {
    let {
      firstSlot,
      lastSlot
    } = _ref3;
    return firstSlot.eqn(rangeOpts[range].firstSlot) && lastSlot.eqn(rangeOpts[range].lastSlot);
