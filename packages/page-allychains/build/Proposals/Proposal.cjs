...
function Proposal(_ref) {
  ...
  const cancelTx = (0, _react.useMemo)(() => api.tx.sudo && hasSudoKey ? api.tx.sudo.sudo(api.tx.proposeAllychain.cancelProposal(id)) : allAccounts.some(a => {
  ...
  const approveTx = (0, _react.useMemo)(() => api.tx.sudo && api.tx.sudo.sudo(api.tx.proposeAllychain.approveProposal(id)), [api, id]);
