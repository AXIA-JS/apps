...
function SignFields(_ref) {
  ...
  (0, _react.useEffect)(() => {
    address && api.derive.balances.account(address).then(_ref2 => {
      let {
        accountNonce
      } = _ref2;
      return setNonce(accountNonce);
    }).catch(console.error);
  }, [address, api]);
  (0, _react.useEffect)(() => {
    onChange({
  ...
  const _setBlocks = (0, _react.useCallback)(function () {
    let blocks = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _util.BN_ZERO;
    return setBlocks(blocks);
  }, []);

  const _setNonce = (0, _react.useCallback)(function () {
    let nonce = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _util.BN_ZERO;
