...
function saveProperties(api, _ref) {
  let {
    name,
    version
  } = _ref;
...
function hasCurrentProperties(api, _ref2) {
  let {
    extension
  } = _ref2;
...
function filterAll(api, all) {
  const extensions = all.map(info => {
    const current = info.known.find(_ref3 => {
      let {
        genesisHash
      } = _ref3;
      return api.genesisHash.eq(genesisHash);
    }) || null; // if we cannot find it as known, or either the specVersion or properties mismatches, mark it as upgradable
