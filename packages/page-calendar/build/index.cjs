...
function CalendarApp(_ref) {
  ...
  const [hasNextMonth, hasNextDay, lastDay] = (0, _react.useMemo)(() => {
    ...
    const hasNextMonth = scheduled.some(_ref2 => {
      let {
        dateTime
      } = _ref2;
      return dateTime >= nextMonthTime;
    });
    const hasNextDay = hasNextMonth || scheduled.some(_ref3 => {
      let {
        dateTime
      } = _ref3;
      return dateTime >= nextDayTime;
    });
    const lastDay = hasNextMonth ? 42 // more than days in month
    : scheduled.reduce((lastDay, _ref4) => {
      let {
        date
      } = _ref4;
  ...
  const _nextMonth = (0, _react.useCallback)(() => setDateState(_ref5 => {
    let {
      dateMonth,
      dateSelected
    } = _ref5;
    return (0, _util.getDateState)((0, _util.nextMonth)(dateMonth), dateSelected);
  }), []);

  const _prevMonth = (0, _react.useCallback)(() => setDateState(_ref6 => {
    let {
      dateMonth,
      dateSelected
    } = _ref6;
    return (0, _util.getDateState)((0, _util.prevMonth)(dateMonth), dateSelected);
  }), []);

  const _nextDay = (0, _react.useCallback)(() => setDateState(_ref7 => {
    let {
      dateSelected
    } = _ref7;
  ...
  const _prevDay = (0, _react.useCallback)(() => setDateState(_ref8 => {
    let {
      dateSelected
    } = _ref8;
    const date = new Date(dateSelected);
  ...
  const _setDay = (0, _react.useCallback)(day => setDateState(_ref9 => {
    let {
      dateMonth
    } = _ref9;
