...
var _useLeaseRanges = require("@axia-js/app-allychains/useLeaseRanges");
...
function createConstDurations(bestNumber, blockTime, items) {
  return items.map(_ref => {
    let [type, duration, additional = _util.BN_ZERO] = _ref;
...
function createCouncilMotions(bestNumber, blockTime, motions) {
  return [['councilMotion', motions.map(_ref2 => {
    let {
      hash,
      votes
    } = _ref2;
...
function createReferendums(bestNumber, blockTime, referendums) {
  return referendums.reduce((result, _ref4) => {
    let {
      index,
      status
    } = _ref4;
...
function createStakingInfo(bestNumber, blockTime, sessionInfo, unapplied, slashDeferDuration) {
  ...
  const slashEras = slashDuration ? unapplied.filter(_ref5 => {
    let [, values] = _ref5;
...
function createScheduled(bestNumber, blockTime, scheduled) {
  return [['scheduler', scheduled.filter(_ref7 => {
    let [, vecSchedOpt] = _ref7;
    return vecSchedOpt.some(schedOpt => schedOpt.isSome);
  }).reduce((items, _ref8) => {
    let [key, vecSchedOpt] = _ref8;
    const blockNumber = key.args[0];
    return vecSchedOpt.filter(schedOpt => schedOpt.isSome).map(schedOpt => schedOpt.unwrap()).reduce((items, _ref9) => {
      let {
        maybeId
      } = _ref9;
...
function createAuctionInfo(bestNumber, blockTime, rangeMax, _ref10) {
  let [leasePeriod, endBlock] = _ref10;
  const blocks = endBlock.sub(bestNumber);
  return [['allychainAuction', [_objectSpread(_objectSpread({}, newDate(blocks, blockTime)), {}, {
...
function addFiltered(state, types) {
  return types.reduce((state, _ref11) => {
    let [typeFilter, items] = _ref11;
    return state.filter(_ref12 => {
      let {
        type
      } = _ref12;
