...
[![maintainability](https://img.shields.io/codeclimate/maintainability-percentage/axia-js/apps?logo=code-climate&style=flat-square)](https://codeclimate.com/github/axia-js/apps)

# @axia-js/apps

A Portal into the AXIA and Substrate networks. Provides a view and interaction layer from a browser.

This can be accessed as a hosted application via https://axia.js.org/apps/ or you can access the IPFS hosted version via https://axia.js.org/apps/ipfs (via hash) or https://dotapps.io (via ipns) to explore any of the supported AXIA and Substrate chains.
...
- [page-js](packages/page-js/) An online code editor with [@axia-js/api](https://github.com/axia-js/api/tree/master/packages/api) access to the currently connected node.
...
In addition the following libraries are also included in the repo. These are to be moved to the [@axia-js/ui](https://github.com/axia-js/ui/) repository once it reaches a base level of stability and usability. (At this point with the framework being tested on the apps above, it makes development easier having it close)
...
To start off, this repo (along with others in the [@axia](https://github.com/axia-js/) family) uses yarn workspaces to organize the code. As such, after cloning dependencies _should_ be installed via `yarn`, not via npm, the latter will result in broken dependencies.

To get started -

1. Clone the repo locally, via `git clone https://github.com/axia-js/apps <optional local path>`
2. Ensure that you have a recent LTS version of Node.js, for development purposes [Node >=10.13.0](https://nodejs.org/en/) is recommended.
3. Ensure that you have a recent version of Yarn, for development purposes [Yarn >=1.10.1](https://yarnpkg.com/docs/install) is required.
4. Install the dependencies by running `yarn`
5. Ready! Now you can launch the UI (assuming you have a local AXIA Node running), via `yarn run start`
...
```
docker run --rm -it --name axia-ui -e WS_URL=ws://someip:9944 -p 80:80 jacogr/axia-js-apps:latest
```

To build a docker container containing local changes -

```
docker build -t jacogr/axia-js-apps .
```
...
IPFS allows sharing files in a decentralized manner in a similar fashion the axia network exchanges blocks. IPFS works best when many nodes seed the same data. Nodes can seed specific data by **pinning** them.

You can pin with the following command:

```
curl -s https://axia.js.org/apps/ipfs/pin.json | jq -jr .IpfsHash | xargs -0 -I CID ipfs pin add --progress CID
```

Here is a script you can save as `/usr/local/bin/axiajs-ipfs-pin.sh`:
...
```
$ /usr/local/bin/axiajs-ipfs-pin.sh
...
SHELL=/bin/bash
HOME=/
0 * * * * /usr/local/bin/axiajs-ipfs-pin.sh >/dev/null 2>&1
```
...
Qmd8UnRQiBobm4qb6dhiC1HoQ7SvwZrWJenoN3JPEV3iiF 480594  axiajs.3af757ad.js
...
* Mac: `~/Library/Application Support/axia-apps/axia-accounts`
* Linux: `~/.config/axia-apps/axia-accounts` (or `$XDG_CONFIG_HOME/axia-apps/axia-accounts` if `$XDG_CONFIG_HOME` is defined)
* Windows: `%APPDATA%\axia-apps\axia-accounts`

For more details on the desktop app, head over to [Electron package README](https://github.com/axia-js/apps/blob/master/packages/apps-electron/README.md).
